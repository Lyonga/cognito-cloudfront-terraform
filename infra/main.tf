...
resource "aws_iam_role" "amplifier_authenticated_role" {
  name = var.cognito_id_role_name
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          "cognito-identity.amazonaws.com:aud": "${aws_cognito_identity_pool.amplifier_identity_pool.id}"
        },
...
resource "aws_iam_role_policy" "authenticated_role_policy" {
  name = var.cognito_id_policy_name
  role = aws_iam_role.amplifier_authenticated_role.id
  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Resource": [
        "${aws_s3_bucket.amplifier_media_bucket.arn}",
        "${aws_s3_bucket.amplifier_media_bucket.arn}/*"
      ]
    }
  ]
...
resource "aws_cognito_identity_pool" "amplifier_identity_pool" {
  identity_pool_name               = "amplifier-identity-pool"
  allow_unauthenticated_identities = false  # Set to true if you want to allow unauthenticated identities

  cognito_identity_providers {
    client_id               = aws_cognito_user_pool_client.amplifiercognito_user_pool_client.id
    provider_name           = aws_cognito_user_pool.amplifier_cognito_user_pool.endpoint
    server_side_token_check = true
  }
}

# Attach the IAM role to the Cognito Identity Pool
resource "aws_cognito_identity_pool_roles_attachment" "amplifier_identity_pool_roles" {
  identity_pool_id = aws_cognito_identity_pool.amplifier_identity_pool.id

  roles = {
    "authenticated" = aws_iam_role.amplifier_authenticated_role.arn
  }
}
